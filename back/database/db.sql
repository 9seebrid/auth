...
values ('1234', '할일1', '할일1 설명', '2021-08-01', false, false, 'koo');

-- 데이터 조회 쿼리
SELECT * FROM task WHERE userId = 'koo' ORDER BY created_at DESC(ASC);
-- ASC 오름차순 , DESC 내림차순

-- 특정 사용자 데이터 필터 조회 쿼리
SELECT * FROM task where userId = '9seebird'

-- 데이터 삭제
DELETE FROM task WHERE _id = '1234'

-- 데이터 업데이트
UPDATE task SET iscompleted = true WHERE _id = '112241732432924616336';
